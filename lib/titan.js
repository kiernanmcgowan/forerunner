  ...
  managerSocket.on('connection', function(socket) {
    ...
    socket.on('manifest', function(data) {
      if (data.manifest && _.isArray(data.manifest)) {
        _currentSlavesAbilities[socketId] = data.manifest;
      } else {
        // wat
...
function getFreeWorker(jobName) {
  ...
  var allWorkers = Object.keys(_currentSlaves);
  ...
  for (var i = 0; i < freeWorkers.length; i++) {
    if (_currentSlavesAbilities[freeWorkers[i]].indexOf(jobName) !== -1) {
...
function assignJobToWorker(jobId, jobName, workerId, payload, callback) {
  ...
  _currentSlaves[workerId].emit('new_job', {id: jobId, type: jobName, payload: payload}, callback);
}
...
function flushQueue() {
  ...
  // look through orphaned jobs first
  console.log(_jobQueue);
  ...
  var eachFn = function(jobId, callback) {
    store.fetchJob(jobId, function(err, jobName, jobData) {
      ...
      if (!workerId) {
        ...
        console.log('no free worker found');
        ...
        var derp = _jobQueue.splice(indexToSplice, 1);
        if (derp != jobId) {
          console.log(derp);
          console.log(jobId);
  ...
  async.each(cloned, eachFn, function() {
