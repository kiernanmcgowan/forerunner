...
var _harbingerDefaults = {
  ...
  //queue: './util/memory_queue',
  queue: 'harbinger-redis',
  // and a console save
  store: './util/console_store',
  // jobs can fail once
  maxFailCount: 1
};
...
function harbinger(opts, callback) {
  ...
  // extent the config
  opts = _.extend(_harbingerDefaults, opts);
  ...
  managerSocket.on('connection', function(socket) {
    ...
    socket.on('disconnect', function() {
      ...
      invalidateJobsOnSocket(socketId, function() {
        console.log('jobs on socket invalidated, id: ' + socketId);
    ...
    socket.on('job_complete', function(payload) {
      ...
      store.storeJob(jobId, jobResult, function(err) {
        ...
        store.removeJob(jobId, function() {
          flushQueue();
        });
      });
    });
...
var flushQueue = _.debounce(function() {
