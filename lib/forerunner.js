...
var _forerunnerDefaults = {
  ...
  proxyLocation: null,
  socketioLogLevel: 1,
  socketioHeartbeatTimeout: 60,
  // suppress logs about flushing the queue / assigning jobs
  verbose: false
};
...
function forerunner(opts) {
  ...
  connectionPool.start({
    ...
    socketioLogLevel: opts.socketioLogLevel,
    socketioHeartbeatTimeout: opts.socketioHeartbeatTimeout
  });
...
// Returns the status of the workers
function status() {
  var statusObj = [];
  _.each(_currentWorker, function(workerSocket) {
    var worker = {};
    // hostname info
    worker.id = workerSocket.id;
    worker.address = workerSocket.handshake.address;
    worker.host = workerSocket.handshake.headers.host;

    // worker type
    worker.abilities = _currentWorkerAbilities[worker.id];

    // job status
    var jobs = [];
    _.each(_jobsOnSocket[worker.id], function(jobObj) {
      jobs.push(_.pick(jobObj, 'id', 'type'));
    });
    worker.current_jobs = jobs;

    statusObj.push(worker);
  });
...
function removeJobFromSocket(socketId, jobId) {
  // check to see if the socket exists first
  // sometimes jobs will block a worker thread and cause a `socket_disconnect` event
  // even though the job may keep running, the manager will clear it from its local memory
  // this is a structural problem that needs to be worked out in future releases
  // but for now, just log a warning.
  if (!_jobsOnSocket[socketId]) {
    logger.warn('Forerunner - removing a job from a socket that does not exist! socketId: ' +
                socketId + ' jobId: ' + jobId);
    return null;
  }
...
function getFreeWorker(jobType) {
  ...
  // find all possible workers that we can send the job off to
  var possibleWorkers = [];
  for (var i = 0; i < freeWorkers.length; i++) {
    ...
    if (_currentWorkerAbilities[freeWorkers[i]] && _currentWorkerAbilities[freeWorkers[i]].indexOf(jobType) !== -1) {
      ...
      possibleWorkers.push(freeWorkers[i]);
    }
  }

  // now randomly select one of the workers
  return _.sample(possibleWorkers);
}
